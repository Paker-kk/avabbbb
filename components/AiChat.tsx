...
const callDeepSeekAPI = async (
  ...
  if (!apiKey) {
    ...
    return language === 'zh' 
      ? '你好！我是 avabbbb 的 AI 助手。请先在上方输入你的 DeepSeek API Key 以启用 AI 对话。'
      : "Hi! I'm avabbbb's AI assistant. Please enter your DeepSeek API Key above to enable AI chat.";
  }
...
export const AiChat: React.FC<AiChatProps> = ({ 
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      const response = await callDeepSeekAPI(chatHistory, language, apiKey, projectsList);
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b-2 border-primary">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              title={language === 'zh' ? '设置 API Key' : 'Set API Key'}
            >
          ...
          <div className="px-5 py-3 bg-primary/5 border-b border-primary/20">
            <div className="flex gap-2">
              <input
                ...
                placeholder={language === 'zh' ? '输入 DeepSeek API Key...' : 'Enter DeepSeek API Key...'}
                className="flex-1 px-3 py-2 bg-cream border border-primary/20 focus:border-primary text-xs text-primary placeholder:text-primary/30 outline-none transition-colors font-mono"
              />
              <button
                onClick={() => {
                ...
                className="px-3 py-2 bg-primary text-cream text-xs font-bold hover:bg-[#E63946] transition-colors"
              >
                {language === 'zh' ? '保存' : 'Save'}
              </button>
            </div>
            <p className="text-[10px] text-primary/40 mt-1.5">
              {language === 'zh' 
                ? 'Key 仅保存在你的浏览器本地，不会上传到服务器'
                : 'Key is stored locally in your browser only, never sent to our server'}
            </p>
          </div>
        )}
